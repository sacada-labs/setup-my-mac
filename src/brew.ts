...
export async function installHomebrew(): Promise<{ success: boolean; error?: string }> {
  try {
    ...
    console.log("This may take a few minutes. Please follow the prompts if any appear.");
    ...
    try {
      ...
      return { success: false, error: `Installation script exited with code ${proc.exitCode}` };
    } finally {
      // Clean up temp file
      try {
...
export async function getInstalledPackages(): Promise<{
  ...
  const [formulas, casks] = await Promise.all([getInstalledFormulas(), getInstalledCasks()]);
