...
export async function installTool(tool: Tool): Promise<InstallationResult> {
	...
	try {
				...
				for (const line of lines) {
					...
					if (progressMatch && downloadStarted) {
						...
						spinner.text = kleur.cyan(
							`Downloading ${tool.name}... ${percent}%${sizeInfo}`,
						);
					...
					if (
						...
						(trimmedLine.includes("%") ||
							trimmedLine.includes("MB") ||
							trimmedLine.includes("GB"))
					) {
						// Update spinner with the latest progress line
						if (spinner.isSpinning && !sudoPrompted) {
							// Extract just the progress info
							const progressInfo = trimmedLine.match(
								/(\d+\.?\d*%|[\d.]+\s*(MB|GB|KB))/i,
							);
							if (progressInfo) {
								spinner.text = kleur.cyan(
									`Downloading ${tool.name}... ${progressInfo[0]}`,
								);
							}
						}
					}
		...
		const errorMessage =
			errorBuffer || outputBuffer || `Exit code ${proc.exitCode}`;
		if (spinner.isSpinning) {
		...
		spinner.fail(
			kleur.red(`✗ Failed to install ${tool.name}: ${errorMessage}`),
		);
...
export async function uninstallTool(tool: Tool): Promise<InstallationResult> {
	...
	try {
		...
		const errorMessage = error instanceof Error ? error.message : String(error);
		spinner.fail(
			kleur.red(`✗ Failed to uninstall ${tool.name}: ${errorMessage}`),
		);
...
export async function uninstallTools(
	tools: Tool[],
): Promise<InstallationResult[]> {
	const results: InstallationResult[] = [];
